...
function extractWeaponDamageDetailsAndCleanText(remainingText: string[]): [DamageDetails | null, string[]] {
  const dpsRegex = /(\d{1,3}(,\d{3})*(\.\d+)?) Damage Per Second/i;
  const damagePerHitRegex = /\[\s*(\d{1,3}(,\d{3})*(\.\d+)?)\s*-\s*(\d{1,3}(,\d{3})*(\.\d+)?)\s*\]\s*Damage per Hit/i;
  ...
  remainingText.forEach((line) => {
    if (line.match(dpsRegex) || line.match(damagePerHitRegex) || line.match(speedRegex)) {
  ...
  const damageDetails: DamageDetails = {
    ...
    min: parseInt(combinedText.match(damagePerHitRegex)?.[1].replace(/,/g, '') || '0'),
    max: parseInt(combinedText.match(damagePerHitRegex)?.[4].replace(/,/g, '') || '0'),
...
function extractArmorValueAndCleanText(remainingText: string[]): [number | null, string[]] {
...
export { extractArmorValueAndCleanText, extractWeaponDamageDetailsAndCleanText, isWeapon };
